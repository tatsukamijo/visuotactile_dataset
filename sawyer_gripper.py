...
class SawyerGripper(px.Robot):
    ...
    #                 ),
    #                 "orientation": spaces.Box(
    #                     low=0, high=0, shape=(4,), dtype=np.float32
    #                 ),
    #             },
    #             "gripper_width": spaces.Box(
    #                 low=0.03, high=0.11, shape=(1,), dtype=np.float32
    ...
    def action_space(self):
        ...
        action_space["end_effector"]["position"] = spaces.Box(
            low=np.array([0.3, -0.85, 0.214]),
            high=np.array([0.85, 0.85, 0.216]),
            shape=(3,),
            dtype=np.float32,
        )
        action_space["end_effector"]["orientation"] = spaces.Box(
            low=0, high=0.01, shape=(4,), dtype=np.float32
        )
